...
public class EffectSelectGui extends MenuFactory {
    ...
    public EffectSelectGui(Player player, int page, EffectType type) {
        ...
        super("&5Select your &d" + (type == EffectType.PARTICLE ? "particle" : (type == EffectType.SOUND ? "sound" : "trail")) + "&5. Page &d" + page, 6);

        List<Integer> darkPurpleGlass = Arrays.asList(0, 1, 7, 8, 36, 37, 43, 44, 45, 46, 52, 53);
        List<Integer> lightPurpleGlass = Arrays.asList(2, 6, 38, 42, 47, 51);
        List<Integer> cyanGlass = Arrays.asList(3, 4, 5, 39, 40, 41, 48, 49, 50);

        int itemsPerPage = 27;
        int totalPages = getPagesByPermission(player, itemsPerPage, type);
        int position = 9;

        for (int i = 0; i < getInventory().getSize(); i++) {
            if (darkPurpleGlass.contains(i)) {
                addItem(new MenuItem(i, new ItemFactory(Material.STAINED_GLASS_PANE, 1, (byte) (10))
                        .setDisplayName(" ")
                        .build()));
            } else if (lightPurpleGlass.contains(i)) {
                addItem(new MenuItem(i, new ItemFactory(Material.STAINED_GLASS_PANE, 1, (byte) (2))
                        .setDisplayName(" ")
                        .build()));
            } else if (cyanGlass.contains(i)) {
                addItem(new MenuItem(i, new ItemFactory(Material.STAINED_GLASS_PANE, 1, (byte) (3))
                        .setDisplayName(" ")
                        .build()));
            }
        }
        ...
        switch (type) {
            ...
            case TRAIL:
                ...
                for (Particle particle : getTrails(page, itemsPerPage)) {

                    Material material = Material.valueOf(CursedEffectsPlugin.getPlugin().getConfig().getString("trails." + particle.name() + ".icon.material"));
                    byte materialData = (byte) (CursedEffectsPlugin.getPlugin().getConfig().getInt("trails." + particle.name() + ".icon.data"));

                    String permission = CursedEffectsPlugin.getPlugin().getConfig().getString("trails." + particle.name() + ".permission");

                    boolean hasPermission = player.hasPermission(permission) || player.hasPermission("cursedeffects.trails.*");
                    boolean isEnabled = data.getParticleTrail() == particle;
                    ...
                    if (hasPermission) {
                        ...
                        if (material == Material.POTION || material == Material.SPLASH_POTION) {
                            ...
                            try {
                                String[] rgb = CursedEffectsPlugin.getPlugin().getConfig().getString("trails." + particle.name() + ".icon.color").split(",");
                                potionFactory.setColour(Color.fromRGB(Integer.parseInt(rgb[0]), Integer.parseInt(rgb[1]), Integer.parseInt(rgb[2])));
                            } catch (Exception e) {
                            }
    ...
    private int getPagesByPermission(Player player, int perPage, EffectType type) {
        ...
        if (type == EffectType.PARTICLE && !player.hasPermission("cursedeffects.particles.*")) {
        ...
        if (type == EffectType.SOUND && !player.hasPermission("cursedeffects.sounds.*")) {
        ...
        if (type == EffectType.TRAIL && !player.hasPermission("cursedeffects.trails.*")) {
            for (Particle trail : CursedEffectsPlugin.getPlugin().getEffectManager().getAvailableTrails()) {
    ...
    private List<Particle> getTrails(int page, int perPage) {
        ...
        try {
            for (int i = ((page - 1) * perPage); i < (((page - 1) * perPage) + perPage); i++) {
                Particle particle = CursedEffectsPlugin.getPlugin().getEffectManager().getAvailableTrails().get(i);
                effects.add(particle);
            }
        } catch (IndexOutOfBoundsException e) { /* Ignored */ }
